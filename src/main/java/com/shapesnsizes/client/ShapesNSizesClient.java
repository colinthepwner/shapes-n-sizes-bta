...
public class ShapesNSizesClient implements ClientModInitializer {
	...
	private static void bindItemModels(ItemModelDispatcher dispatcher) {
		int bound = 0;
		for (Item item : new Item[]{ShapesItems.BROWNIE_BIG, ShapesItems.BROWNIE_SMALL,
	...
	private static int bindPainted(ItemModelDispatcher dispatcher, Item item, String texturePrefix) {
		if (item == null) return 0;
		dispatcher.addDispatch(new ItemModelSizedDoorPainted(item, texturePrefix));
		if (dispatcher.hasDispatch(item)) return 1;
		ShapesNSizes.LOGGER.warn("No item model bound for {}; it will be invisible.", item.getKey());
		return 0;
	}
	...
	private static void bindBlockModels(BlockModelDispatcher dispatcher) {
		int bound = 0;
		bound += bindDoor(dispatcher, "short", new Block[]{ShapesDoors.SHORT}, false);
		bound += bindDoor(dispatcher, "tall", ShapesDoors.TALL, false);
		bound += bindDoor(dispatcher, "verytall", ShapesDoors.VERY_TALL, false);
		bound += bindDoor(dispatcher, "short", ShapesDoors.SHORT_PAINTED, true);
		bound += bindDoor(dispatcher, "tall", ShapesDoors.TALL_PAINTED, true);
		bound += bindDoor(dispatcher, "verytall", ShapesDoors.VERY_TALL_PAINTED, true);
		ShapesNSizes.LOGGER.info("Bound {} of 16 door segment models.", bound);
	}

	private static int bindDoor(BlockModelDispatcher dispatcher, String door, Block<?>[] segments, boolean painted) {
		...
		for (int i = 0; i < segments.length; ++i) {
			...
			dispatcher.addDispatch(painted
				? new BlockModelSizedDoorPainted<>(block, ShapesDoors.MODEL_DIR, segment)
				: new BlockModelSizedDoor<>(block, ShapesDoors.MODEL_DIR, segment));
